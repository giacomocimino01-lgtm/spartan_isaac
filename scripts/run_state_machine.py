...
class SPARTANStateMachine:
    ...
    def __init__(self, env):
        ...
        # self.target_pos = {
        #     "right_arm": torch.tensor([[0.30, -0.05, 0.76]], device=self.device).repeat(env.num_envs, 1),
        #     "left_arm":  torch.tensor([[0.45, -0.05, 0.76]], device=self.device).repeat(env.num_envs, 1)
        # }
    ...
    def set_new_triplet(self, verb: str, subject: str, target: str):
        ...
        self.current_triplet = {"verb": verb, "subject": subject, "target": target}
        self.sub_state = "COMPUTE_TARGET_POS"
        self.step_counter = 0
    ...
    def get_action(self):
        ...
        for arm_name in ["right_arm", "left_arm"]:
            ...
            # Troviamo dove si trova la pinza ORA
            ...
            if torch.any(self.target_pos[arm_name] != 0):
                # Errore = Destinazione in memoria - Punta Attuale
                errore = self.target_pos[arm_name] - tip_pos_w
                idle_action[:, 0:3] = errore
            else:
                # Nessun bersaglio iniziale: comandiamo un piccolo spostamento relativo verso l'alto per non farlo cadere
        ...
        # --- LOGICA ATTIVA (Relativa) ---
        if self.current_triplet is not None and self.sub_state != "IDLE":
            verb = self.current_triplet["verb"]
            subject = self.current_triplet["subject"]
            ...
            # Posizione attuale della pinza (ci serve ad ogni frame per calcolare l'errore)
            body_idx = robot.find_bodies("psm_tool_tip_link")[0][0]
            tip_pos_w = robot.data.body_pos_w[:, body_idx]

            # === AZIONE: REACH ===
            if verb == "reach":
                if self.sub_state == "COMPUTE_TARGET_POS":
                    self.sub_state = "APPROACH_ABOVE"
                    self.step_counter = 0

                elif self.sub_state == "APPROACH_ABOVE":
                    self.target_pos[subject] = self.get_target_coordinates(self.current_triplet["target"], subject)
                    
                    # Destinazione: Target + offset di sicurezza in Z
                    destinazione = self.target_pos[subject] + torch.tensor([[0.0, 0.0, self.SAFE_Z_OFFSET]], device=self.device)
                    # Comandiamo l'ERRORE (Destinazione - Attuale)
                    active_arm[:, 0:3] = destinazione - tip_pos_w
                    
                    if self.step_counter > 150:
                        self.sub_state = "DESCEND"

                elif self.sub_state == "DESCEND":
                    self.target_pos[subject] = self.get_target_coordinates(self.current_triplet["target"], subject)
                    ...
                    # Controllo Disaccoppiato (Cilindro)
                    ...
                    if distanza_xy[0] < self.TOLERANCE_XY and distanza_z[0] < self.TOLERANCE_Z:
                        print(f"[SM] Target agganciato! Err_XY: {distanza_xy[0].item():.4f}m | Err_Z: {distanza_z[0].item():.4f}m")
                        print(f"[SM] Target agganciato! Err_XY: {tip_pos_w}m")
                        print(f"[SM] Target agganciato! Err_XY: {destinazione_reale}m")  
                        self.sub_state = "SETTLE"
                        self.step_counter = 0

                elif self.sub_state == "SETTLE":
                    destinazione_reale = self.target_pos[subject] + torch.tensor([[0.0, 0.0, self.GRASP_Z_OFFSET]], device=self.device)
                    # Comandiamo l'ERRORE per mantenere la posizione
                    active_arm[:, 0:3] = destinazione_reale - tip_pos_w
                    
                    if self.step_counter > 15:
                        self.last_target[subject] = self.current_triplet["target"]
            ...
            # === AZIONE: GRASP ===
                ...
                if self.sub_state == "COMPUTE_TARGET_POS":
                    self.sub_state = "CLOSE_GRIPPER"
                    self.step_counter = 0

                elif self.sub_state == "CLOSE_GRIPPER":
                    ...
                    if self.step_counter > 100:
                        # 1. Agganciamo l'oggetto al braccio corrente
                        oggetto_da_prendere = self.current_triplet["target"]
                        self.attached_target[subject] = oggetto_da_prendere
                        
                        # ==========================================
                        # LA MAGIA DELL'HANDOVER (SCAMBIO)
                        # ==========================================
                        # Controlliamo l'ALTRO braccio. Se stava tenendo questo stesso oggetto, 
                        # costringiamolo a mollarlo!
                        ...
                        if self.attached_target[altro_braccio] == oggetto_da_prendere:
                            ...
                            # Opzionale: diciamo all'altro braccio di aprire la pinza visivamente
                            self.current_gripper_state[altro_braccio] = 1.0 
                            
                        self.sub_state = "LIFT_UP"
                
                elif self.sub_state == "LIFT_UP":
                    active_gripper[:] = self.current_gripper_state[subject]
            ...
            # === AZIONE: RELEASE ===
                ...
                if self.sub_state == "COMPUTE_TARGET_POS":
                    self.sub_state = "APPROACH_PEG"
                    self.step_counter = 0

                elif self.sub_state == "APPROACH_PEG":
                    self.current_gripper_state[subject] = -1.0 
                    active_gripper[:] = self.current_gripper_state[subject]
                    
                    target_cmd = self.current_triplet["target"]
                    ...
                    if target_peg is not None:
                        ...
                        if self.step_counter > 120:
                            self.sub_state = "DESCEND_TO_PEG"
                            self.step_counter = 0
                    else:
                        # Rilascio a vuoto: tieni fermo
                        active_arm[:, 0:3] = self.target_pos[subject] - tip_pos_w
                        if self.step_counter > 50:
                            self.sub_state = "OPEN_GRIPPER"
                            self.step_counter = 0

                elif self.sub_state == "DESCEND_TO_PEG":
                    self.current_gripper_state[subject] = -1.0 
                    active_gripper[:] = self.current_gripper_state[subject]
                    ...
                    # Sostituiamo solo la Z con l'altezza del tavolo
                ...
                elif self.sub_state == "OPEN_GRIPPER":
                    self.current_gripper_state[subject] = 1.0 # Apri
                    active_gripper[:] = self.current_gripper_state[subject]
                    ...
                    if self.step_counter == 10:
                        ...
                        if target_peg is not None and obj_name is not None:
                            ...
                            # FIX: Leggiamo le coordinate vere (senza offset) direttamente dalla scena
                    ...
                    if self.step_counter > 80:
                        if target_peg is not None:
                            self.sub_state = "RETREAT"
                        else:
                            self.sub_state = "IDLE"
                        self.step_counter = 0

                elif self.sub_state == "RETREAT":
                    self.current_gripper_state[subject] = 1.0
                    active_gripper[:] = self.current_gripper_state[subject]
    ...
    def _build_ui(self):
        with self.window.frame:
            with ui.VStack(spacing=10):
                ui.Label("VERB ATTUALE:", style=self.style_header)
                ...
                # Aggiunto "SETTLE" alla lista degli stati!
                all_states = [
...
def main():
    ...
    while simulation_app.is_running():
        ...
        # 2. Ascolto Comandi
        if trigger_comando and state_machine.sub_state == "IDLE":
        ...
        for arm_name, attached_obj in state_machine.attached_target.items():
            if attached_obj is not None:
                ...
                # Manteniamo l'anello orizzontale
        ...
        # 6. Aggiornamento GUI
        verb = state_machine.current_triplet["verb"] if state_machine.current_triplet else None
        sm_gui.update(current_verb=verb, current_sub_state=state_machine.sub_state)
